...
@section('title', 'BAYES | Hasil Diagnosa')
@section('content')
                    ...
                    <div class="col-auto ms-auto d-print-none">
                        <form action="/perhitungan-simpan" method="post">
                            ...
                            @foreach ($bayes as $item)
                                ...
                                <input type="hidden" name="hipotesa[{{ $i }}][persentase_probabilitas]"
                                    value="@php echo($item['persentase_probabilitas']) @endphp">
                                @php
                                    $j = 0;
                                @endphp
                                @foreach ($item['solusi'] as $item_solusi)
                                    <input type="hidden"
                                        name="hipotesa[{{ $i }}][solusi][{{ $j }}]"
                                        value="@php echo($item['solusi'][$j]) @endphp">
                                    @php
                                        $j++;
                                    @endphp
                                @endforeach
                            ...
                            <tbody>
                                @foreach ($bayes as $item)
                                    <tr>
                                        ...
                                        {{-- <td style="color:#4299E1;">{{ $item['kode_penyakit'] }}</td> --}}
                                        <td><span
                                                style="color:#4299E1;">{{ '(' . $item['kode_penyakit'] . ') ' }}</span>{{ $item['penyakit'] }}
                                        </td>
                                        <td>{{ $item['probabilitas'] }}</td>
                                        <td>{{ $item['persentase_probabilitas'] }} %</td>
                                        <td style="max-width: 500px;">
                                            @if (isset($item['solusi']) && count($item['solusi']) > 0)
                                                @foreach ($item['solusi'] as $item_solusi)
                                                    <ul>
                                                        <li>{{ $item_solusi }}</li>
                                                    </ul>
                                                @endforeach
                                            @else
                                                <span class="badge bg-pink">Solusi belum ditambahkan</span>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
